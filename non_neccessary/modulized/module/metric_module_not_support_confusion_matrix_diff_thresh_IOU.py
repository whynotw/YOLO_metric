...
class MAPMetric():
    ...
    def get_confusion(self,
        ...
        # TODO how to calculate confusion matrix for different thresh_IOU?
        # TODO be careful! IOU has some problem!?
        matrix_confusion = np.zeros((self.number_classes+1,self.number_classes+1))
        number_prediction_all = [None]*self.number_classes
        for no_class_prediction in range(self.number_classes):
            above_thresh_confidence = np.array(self.scores_all[no_class_prediction]) >= thresh_confidence
            above_thresh_IOU        = np.array(  self.IOUs_all[no_class_prediction]) >= thresh_IOU
            above_thresh = above_thresh_confidence & above_thresh_IOU
            above_thresh = above_thresh_confidence
            number_prediction_all[no_class_prediction] = sum(above_thresh_confidence)
            confusions_single = np.array(self.confusions_all[no_class_prediction])
            labels = confusions_single[above_thresh]
            for no_class_groundtruth in labels:
                matrix_confusion[no_class_groundtruth,no_class_prediction] += 1
            labels = confusions_single[np.logical_not(above_thresh)]
            for no_class_groundtruth in labels[labels >= 0]:
                matrix_confusion[no_class_groundtruth,-1] += 1
        for no_class_groundtruth in self.confusions_all[-1]:
            matrix_confusion[no_class_groundtruth,-1] += 1
        IOUs_avg = []
        for no_class in range(self.number_classes):
        ...
        if not conclude:
           return # TODO return what you want
        #number_prediction_all = [len(n) for n in self.confusions_all]
        #number_prediction_all = []
        #for no_class in range(self.number_classes):
        #    number_prediction_all.append(self.scores_all[no_class]>=thresh_co)
        # print the results
