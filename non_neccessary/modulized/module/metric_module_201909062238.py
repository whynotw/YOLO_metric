...
class MAPMetric():
    ...
    def _sort(self):
        for no_class in range(self.number_classes):
            #print(len(self.scores_all[no_class]),"correct")
    ...
    #def _sort(self):
    #    #infos_last = np.array( self.infos_all[-1] )
    #    for no_class in range(self.number_classes):
    #        infos = np.array( self.infos_all[no_class] )
    #        matched = ( infos[:,0] == no_class )
    #        #print(sum(matched))
    #        self.scores_all[no_class] = list(infos[matched,1])
    #        self.IOUs_all[no_class] = list(infos[matched,2])
    #        #matched = ( infos_last[:,0] == no_class )
    #        #print(sum(matched))
    #        #self.scores_all[no_class] += list(infos_last[matched,1])
    #        #self.IOUs_all[no_class] += list(infos_last[matched,2])
    #        infos = np.array( self.infos_all[-1] )
    #        self.scores_all[no_class] += list(infos[:,1])
    #        self.IOUs_all[no_class] += list(infos[:,2])
    #        print(len(self.IOUs_all[no_class]))
    #        if len(self.IOUs_all[no_class]) != 0:
    #            self.IOUs_all[no_class],\
    ...
    def get_confusion(self,
        ...
        for no_class,name in enumerate(self.names_class):
            print(("%*s   precision: %6.2f %%"
                      ...
                      "     avg IOU: %6.2f %%")%(\
                length_name,name,
                1e2*matrix_confusion[no_class,no_class]/self.number_prediction_all[no_class],
