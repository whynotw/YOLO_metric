...
class ObjectDetectionMetric():
    ...
    def get_confusion(self,
        ...
        content = ""
        spacing1 = "- "*max((int(7+((length_name+3)*(self.number_classes+3))/2)),
                           length_name+33)
        content += "%s\nConfusion Matrix\n%s\n"%(spacing1,spacing1)
        content += ("thresh_confidence: %f\n"%thresh_confidence).rstrip("0")
        content += ("thresh_IOU       : %f\n"%thresh_IOU).rstrip("0")
        matrix_confusion = np.uint32(matrix_confusion)
        spacing2 = " "*(length_name+3+12)
        content += "\n%*sPrediction\n"%(12+(len(spacing2)-10)/2,"")
